...
import { assertSufficientFunds } from './balance.js';
...
export async function postBalancedTransfer(client: PoolClient, input: {
  ...
  if (!source || !destination) throw new Error('Ledger account not found');
  ...
  await assertSufficientFunds(client, source.id, input.amountMinor);
  ...
  const balanced = await client.query<{ balanced: boolean }>(
    `SELECT journal_is_balanced($1) AS balanced`, [journalId],
  );
  if (!balanced.rows[0]?.balanced) throw new Error('Ledger journal is not balanced');
}
